var NavTree = function (selector, options) {
  ...
  var props = {
    ...
    groupOpenIconClass:
      options.groupOpenIconClass && options.groupOpenIcon
        ? options.groupOpenIconClass
        : "fas",
    groupOpenIcon:
      options.groupOpenIconClass && options.groupOpenIcon
        ...
        : "fa-chevron-down",
    groupCloseIconClass:
      options.groupCloseIconClass && options.groupCloseIcon
        ? options.groupCloseIconClass
        : "fas",
    groupCloseIcon:
      options.groupCloseIconClass && options.groupCloseIcon
        ? options.groupCloseIcon
        : "fa-chevron-right",
    linkIconClass:
      options.linkIconClass && options.linkIcon ? options.linkIconClass : "fas",
  ...
  var init = function () {
    ...
    element
      .querySelectorAll("ul:not(.extra)")
      .forEach((ul) => ul.classList.add("nav", "flex-column")); // add bs5 classes to lists
    ...
    element.querySelectorAll("li:not(.extra)").forEach((li) => {
      ...
      if (li.querySelector("ul:not(.extra)")) {
        // check list item has a ul object
        ...
        a.prepend(
          getIconWithWrapper(props.groupOpenIconClass, props.groupOpenIcon)
        );
        ...
        // } else {
        //   a.prepend(
        //     getIconWithWrapper(props.groupCloseIconClass, props.groupCloseIcon)
        //   );
        // }
        ...
        collapsable.addEventListener("show.bs.collapse", (e) => {
          e.target.parentElement
            .querySelector(".tree-icon i")
            .classList.replace(props.groupCloseIcon, props.groupOpenIcon);
          e.target.parentElement
            .querySelector(".tree-icon i")
            .classList.replace(
              props.groupCloseIconClass,
              props.groupOpenIconClass
            );
        });
        ...
        collapsable.addEventListener("hide.bs.collapse", (e) => {
          e.target.parentElement
            .querySelector(".tree-icon i")
            .classList.replace(props.groupOpenIcon, props.groupCloseIcon);
          e.target.parentElement
            .querySelector(".tree-icon i")
            .classList.replace(
              props.groupOpenIconClass,
    ...
    if (props.searchable) {
      ...
      search_parent.firstChild.addEventListener("keyup", (e) => {
        ...
        if (e.target.value !== "") {
          ...
          nodes
            .filter((node) =>
              node.querySelector("a").classList.contains("tree-group-link")
            )
            .forEach((node) => {
              if (
                Array.from(node.querySelectorAll("li:not(.extra)")).filter(
                  (li) => !li.classList.contains("d-none")
                ).length > 0
              ) {
